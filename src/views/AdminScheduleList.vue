<template>
  <Form :model="formItem" :label-width="80" style="margin: 20px 0 20px 40px">
      ...
      <Col span="6">
        <FormItem label="选择电影" style="width: 250px">
          <Select v-model="formItem.selectMovie">
            <Option v-for="item in MovieNameList" :value="item.value" :key="item.value">{{ item.label }}</Option>
          </Select>
        </FormItem>
      </Col>
    </Row>
    ...
    <FormItem label="选择日期">
      <Row>
        <Col span="5">
          <DatePicker type="daterange" confirm placement="bottom-end" placeholder="请选择日期范围"
                      v-model="formItem.date"></DatePicker>
        </Col>
        <Col span="1" style="text-align: center">-</Col>
        <Col span="5">
          <TimePicker format="HH:mm" type="timerange" placement="bottom-end"
                      placeholder="请选择时间范围（可选）" v-model="formItem.time" style="width: 168px"></TimePicker>
        </Col>
      </Row>
    </FormItem>
    <div class="card" style="margin-top: 20px">
      ...
      <div class="schedule-container">
        <ul class="schedule-time-line">2019/9/1
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      askForMovie() {
        var _this = this;
        axios.get('http://172.28.202.212:8080/GetMovieList')
          .then(function (response) {
            _this.data = response;
            for (var i = 0; i < _this.data.length; i++) {
              _this.MovieNameList.push({
                label: _this.data[i].name,
                value: _this.data[i].id,
              })
            }
